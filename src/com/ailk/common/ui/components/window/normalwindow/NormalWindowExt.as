package com.ailk.common.ui.components.window.normalwindow
{
	import com.ailk.common.ui.components.window.normalwindow.event.NormalWindowExtEvent;
	...
	[Event(name="close", type="com.ailk.common.ui.components.window.normalwindow.event.NormalWindowExtEvent")]
	[Event(name="maximize", type="com.ailk.common.ui.components.window.normalwindow.event.NormalWindowExtEvent")]
	[Event(name="minimize", type="com.ailk.common.ui.components.window.normalwindow.event.NormalWindowExtEvent")]
	[Event(name="restore", type="com.ailk.common.ui.components.window.normalwindow.event.NormalWindowExtEvent")]
	[Event(name="focusstart", type="com.ailk.common.ui.components.window.normalwindow.event.NormalWindowExtEvent")]
	[Event(name="focusend", type="com.ailk.common.ui.components.window.normalwindow.event.NormalWindowExtEvent")]
	[Event(name="config", type="com.ailk.common.ui.components.window.normalwindow.event.NormalWindowExtEvent")]
	
	/**
	 * 保持基本拖拽功能窗口面板，最大化、最小化、还原、关闭操作由外部实现，内部不作任何控制
	 ...
	 */
	public class NormalWindowExt extends Panel
	{
		...
		public function NormalWindowExt()
		{
			super();
		}
		...
		private function focusHandle():void
		{
			if (hasFocus == true)
			{
				...
				dispatchEvent(new NormalWindowExtEvent(NormalWindowExtEvent.FOCUS_START));
			}
			else
			{
				this.alpha=0.6;
				dispatchEvent(new NormalWindowExtEvent(NormalWindowExtEvent.FOCUS_END));
			}
		}
		...
		{
			if (windowState != NormalWindowExtState.MAXIMIZED)
			{
		...
		{
			windowState == NormalWindowExtState.MINIMIZED;
			dispatchEvent(new NormalWindowExtEvent(NormalWindowExtEvent.MINIMIZE));
		}
		/**
		 * 窗口还原时调用
		 */
		...
		{
			windowState == NormalWindowExtState.NORMAL;
			dispatchEvent(new NormalWindowExtEvent(NormalWindowExtEvent.RESTORE));
		}
		...
		{
			windowState == NormalWindowExtState.MAXIMIZED;
			dispatchEvent(new NormalWindowExtEvent(NormalWindowExtEvent.MAXIMIZE));
		}
		...
		{
			dispatchEvent(new NormalWindowExtEvent(NormalWindowExtEvent.CONFIG));
		}
		...
		{
			dispatchEvent(new NormalWindowExtEvent(NormalWindowExtEvent.CLOSE));
		}
		...
		{
			if (windowState == NormalWindowExtState.NORMAL)
			{
		...
		public function set windowState(windowState:int):void
		{
